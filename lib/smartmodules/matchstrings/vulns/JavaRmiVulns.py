...
vulns_match['java-rmi'] = {
    ...
    'jexboss': {
        #r'\[\*\]\s*Checking\s+(?P<m1>.*):\s*\[\s*(EXPOSED|VULNERABLE)\s*\]': 'JBoss: $1 exposed/vulnerable',
        r'Captured ICMP traffic:[\s\S]*ICMP echo request.*\n.*ICMP echo reply': 'Java-RMI Deserialize RCE in Tomcat JMX (CVE-2016-3427, CVE-2016-8735)',
    },
    'jmxbf': {
        '###SUCCESS### - We got a valid connection for: :': 'JMX Authentication disabled',
    },
    'metasploit': {
        r'java_rmi_server[\s\S]*session [0-9]+ opened': 'Java RMI Server Insecure Default Configuration Java Code Execution',
        r'java_jmx_server[\s\S]*session [0-9]+ opened': 'Java JMX Server Insecure Configuration Java Code Execution',
    },
    'sjet': {
        'Successfully loaded MBeanSiberas': 'Java JMX Server Insecure Configuration Java Code Execution',
        r'Loaded de\.siberas\.lab\.SiberasPayload': 'Java JMX Server Insecure Configuration Java Code Execution',
